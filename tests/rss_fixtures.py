...
def rss(feed_title: str, items: list[tuple]) -> str:
    """items: (title, published) or (title, published, itunes_duration)."""
    parts = []
    for item in items:
        ...
        parts.append(
            ...
            f"<pubDate>{format_datetime(pub)}</pubDate>{dur_tag}"
    ...
    return (
        ...
        f"<channel><title>{feed_title}</title>{''.join(parts)}</channel></rss>"
    )
